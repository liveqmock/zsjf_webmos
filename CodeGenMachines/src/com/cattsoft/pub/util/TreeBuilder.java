...
public class TreeBuilder {
	...
	public static String buildTreeB(List lList, List nList) {
		return buildTreeB(lList, nList, null);
	}
	...
	public static String buildTree(List list, HttpServletRequest req) {
		Object show = null;
		if (req != null) {
	...
	public static String buildTreeB(List lList, List nList, HttpServletRequest req) {
		Object show = null;
		if (req != null) {
			show = req.getAttribute("showTreeNode");
		}
	...
	public static Element buildNodeB(TreeBean tb) {
		Element div = DocumentHelper.createElement("div");
		/**
		 * 父节点可能为空，增加判定条件
		 */
		...
		span.addAttribute("style",
				"cursor:default;text-align:left;color:black;font-size:9pt;margin:0;");
		Element span2 = span.addElement("span");
		span2.addAttribute("id", tb.getId() + "bullet");
		span2.addAttribute("onclick", "folder(this," + tb.getId() + ")");
		span2.addAttribute("onmouseout", "mouseOutB(this)");
		span2.addAttribute("onmousedown", "mouseDown(this)");
		span2.addAttribute("onmouseup", "mouseUp(this)");
		span2.addAttribute("style", "cursor:hand;padding:0;margin-left:-3;text-align:left;");
		Element img = span2.addElement("img");
		...
		label.addAttribute("style", "font-size:9pt;text-align:left;margin-left:-3");
		label.addText(tb.getName());
		Element div2 = span2.addElement("div");
		div2
				.addAttribute("style",
						"visibility: hidden;position: absolute;border-width:1;border-style:ridge;background:#cccccc;");
		div2.addAttribute("id", tb.getId() + "bullet" + "#");
		div2.setText("#");
		span2.addAttribute("onmouseover", "mouseOverB(this)");
	...
	public static Element buildLeafB(TreeBean tb) {
		Element div = DocumentHelper.createElement("div");
		div.addAttribute("isNode", "N");
		div.addAttribute("serial", tb.getId().toString());
		// if (log.isDebugEnabled()) {
	...
	// 部分数据被选中的树形结构
	public static String buildTreeCheckBoxPart(List lList, List nList, String type) {
		String retStr = null;
		if (!"0".equals(type) && !"1".equals(type) && !"2".equals(type)) {
			type = "0";
		}
		retStr = buildTreeCheckBoxPart(lList, nList, null, type);
		return retStr;
	}

	public static String buildTreeCheckBoxPart(List lList, List nList, HttpServletRequest req,
			String type) {
		Object show = null;
		if (req != null) {
			show = req.getAttribute("showTreeNode");
	...
	public static Element buildNodeCheckBoxPart(TreeBean tb) {
		Element div = DocumentHelper.createElement("div");
		/**
		 * 父节点可能为空，增加判定条件
		 */
		...
		chek.addAttribute("name", "nodeCheckList");
		chek.addAttribute("value", tb.getId() + "");
		// 判断这个节点的checkbox是否被选中
		System.out.println(tb.getValue());
		if (tb.getValue().equals("Y")) {
			chek.addAttribute("checked", "true");
		}
		Element span2 = span.addElement("span");
		span2.addAttribute("onclick", "folder(this," + tb.getId() + ")");
		span2.addAttribute("onmouseover", "mouseOver(this)");
