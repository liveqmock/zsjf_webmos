...
//		装载DAO实现的配置
		...
		LogUtil.debug(log, "开始装载配置文件:"+daoConfigFileName);
		...
		if(url != null){
			...
			LogUtil.debug(log,"系统路径："+classPath);
		}
		String jarPath = "";
		...
		if(jarFilesStrAry!=null){
			for(int i=0;i<jarFilesStrAry.length;i++){
				if(!StringUtil.isBlank(jarFilesStrAry[i])){
					...
					LogUtil.debug(log, "读取jar文件："+jarPath);
					try {
						JarFile jarFile = new JarFile(jarPath);
						jarFiles.add(jarFile);
						LogUtil.debug(log, "读取jar文件："+jarFile);
					} catch (IOException e) {
						LogUtil.debug(log, "读取jar文件失败！"+jarPath);
					}
				}
			}
		}
		...
		for (int i = 0; i < interfaceCatalogAry.length; i++) {	
			...
			LogUtil.debug(log, "装载DAO包：" + interfaceCatalogAry[i]);
...
//			遍历jar文件，过滤其中的DAO接口文件名列表
			...
			if(interfaceDAOJar != null) {
				...
				LogUtil.debug(log, "Jar文件中找到dao接口类:"+interfaceClassNameList.size()+"个。");
			}else{
				LogUtil.debug(log, "Jar文件中未找到dao接口类！");
			}
			
			//LogUtil.debug(log,"文件相对路径："+ relativePath);
			URL pathUrl = JdbcDAOConfig.class.getResource("/" + relativePath);
			if (null != pathUrl) {
				String path = pathUrl.getPath();
				//LogUtil.debug(log,"文件绝对路径："+ path);
				...
				if (null == daoInterfaceFiles || daoInterfaceFiles.length == 0) {
					LogUtil.debug(log, "未找到要装载的DAO对象！");
					continue;
				}
	...
	private Map loadDAOImpl(String interfaceCatalog,List interfaceClassNameList){
		...
		for (int j = 0; j < interfaceClassNameList.size(); j++) {
			...
			try {
				...
				//LogUtil.debug(log, "DAO接口名称:"+DAOInterfaceName);
				//LogUtil.debug(log, "DAO实现名称:"+DAOImplName);
				// 实现名称加上Impl后缀
				DAOImplName = DAOImplName + DAO_IMPL_SUFFIX;
				Object impl = ObjectUtil.createByName(DAOImplName);
				// LogUtil.debug(log, "成功装载DAO:"+DAOImplName);
				// LogUtil.debug(log, "map key:"+DAOInterfaceName);
				DAOImpls.put(DAOInterfaceName, impl);
				countDao++;
			} catch (UtilException e) {
				LogUtil.error(log, DAOImplName + "文件装载错误!"+e.getMessage());
				continue;
			}
		}
		LogUtil.debug(log, "成功装载" + interfaceCatalog + "的DAO实现" + countDao + "个。");
		return DAOImpls;
	}
